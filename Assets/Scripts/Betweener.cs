...
public class Betweener: MonoBehaviour {
	...
	private IEnumerator Maths() {
		...
		GameObject go = GameObject.Find(miniGames[minigameChoice].name + "/Directions");
		Text directions = go.GetComponent<Text>();
	...
	private IEnumerator AnnounceBonusWinner(int traderNum) {
		...
		string winnerText = GlobalVariables.S.shortTeamNames[traderNum] + " HAS WON THE BONUS!";
